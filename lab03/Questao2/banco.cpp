...
*/
void
banco::criarConta(std::string agencia, int numero, double limite,double saldo){
	conta a(agencia,numero,limite,saldo);
	...
	for(size_t i=0; i<clientes.size();i++)
	{
		if(conta==clientes[i].getNumero())
		{
			if(valor<=clientes[i].getLimite())
			{
				...
				{
					std::cout<<"Desculpe mas você não tem essa quantia em sua conta"<<std::endl;
				}
			}
			else
			{
